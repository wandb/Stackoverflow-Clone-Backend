...
exports.create = async (newComment, result) => {
  await CommentsModel.create({
    ...
    .then(response => {
      result(null, responseHandler(true, 200, 'Comment Added', response.id));
    })
    .catch(error => {
      console.log(error);
      result(
        responseHandler(
          false,
          500,
          'Some error occurred while adding the comment.',
          null
        ),
        null
      );
...
exports.remove = async (id, result) => {
  await CommentsModel.destroy({
    ...
    .catch(error => {
      ...
      result(
        responseHandler(false, 404, "This comment doesn't exists", null),
        null
      );
    });
};
...
exports.removePostComments = async (postId, t) => {
  await CommentsModel.destroy({where: {post_id: postId}}, {transaction: t})
...
exports.retrieveAll = async (postId, result) => {
  ...
  if (utils.conditional.isArrayEmpty(queryResultMap)) {
    ...
    return result(
      responseHandler(false, 404, 'There are no comments', null),
      null
    );
  }
